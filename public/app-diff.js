(function() {
  ...
  $.get('/choosen', {name: window.location.search.substr(1).split("&")[0].split("=")[1]}, (data1) => {
    $.get('/choosen', {name: window.location.search.substr(1).split("&")[1].split("=")[1]}, (data2) => {
      loadDiagram('left', { xml: data1 });
      loadDiagram('right', { xml: data2 });
    })
  })
